module Guard
  class Deprecator
    ...
    GET_GUARD_CLASS_DEPRECATION = <<-EOS.gsub(/^\s*/, '')
      Starting with Guard v2.0 'Guard.get_guard_class(name, fail_gracefully =
    ...
    GUARD_GUARD_DEPRECATION = <<-EOS.gsub(/^\s*/, '')
      Starting with Guard v2.0, Guard::%s should inherit from Guard::Plugin instead of Guard::Guard.
    ...
    WATCH_ALL_MODIFICATIONS_DEPRECATION = <<-EOS.gsub(/^\s*/, '')
      Starting with Guard v1.1 the 'watch_all_modifications' option is removed
      and is now always on.
    EOS

    # Deprecator message for the `no_vendor` start option
    NO_VENDOR_DEPRECATION = <<-EOS.gsub(/^\s*/, '')
      Starting with Guard v1.1 the 'no_vendor' option is removed because the
