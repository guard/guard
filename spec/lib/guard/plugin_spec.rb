...
RSpec.describe Guard::Plugin do
  ...
  before do
    allow(ENV).to receive(:[]).with("GEM_REQUIREMENT_GUARD-BAR")
  end
  ...
  describe "#initialize" do
    ...
    it "assigns the defined options" do
      ...
      expect(Guard::Bar.new(engine: engine, options: opts).options).to eq opts
    end
    ...
    context "with a callback" do
      it "adds the callback" do
        ...
        plugin = Guard::Bar.new(engine: engine, options: { callbacks: callbacks })
