...
RSpec.describe Guard::Notifier::Detected do
  class FakeDetectEnvironment < Guard::Internals::Environment
    attr_accessor :notifiers
  end

  subject { described_class.new(supported) }

  let(:env) { instance_double(FakeDetectEnvironment) }
  ...
  before do
    ...
    allow(env).to receive(:create_method).with(:notifiers)
    allow(env).to receive(:create_method).with(:notifiers=)
